...
import { useCreateAhServiceProvider, useUpdateAhServiceProvider } from '@/hooks/use-allied-health';
...
export function AddServiceProviderDialog({ open, onOpenChange, provider }: { open: boolean, onOpenChange: (open: boolean) => void, provider?: any }) {
  ...
  const { mutate: createProvider, isPending: isCreating } = useCreateAhServiceProvider();
  const { mutate: updateProvider, isPending: isUpdating } = useUpdateAhServiceProvider();
