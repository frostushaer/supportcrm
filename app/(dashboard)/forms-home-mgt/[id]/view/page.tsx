...
export default function ViewHomeFormPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const resolvedParams = use(params);
  const { data, isLoading } = useHomeForm(resolvedParams.id);
  
  // State to track additional dynamic "Add Other" rows
    ...
    <div className="flex flex-col gap-6 p-6 max-w-5xl mx-auto w-full">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-[var(--color-text-secondary)]">Home Management</span>
        </div>
            ...
            {section.subSections?.map((subSection) => {
              ...
              return (
                <div key={subSection.id} className="mb-6 last:mb-0 bg-gray-50/50 p-4 rounded-md border border-gray-100">
                  ...
                  {/* Render extra "Add Other" rows dynamically */}
                  {rows.length > 0 && (
