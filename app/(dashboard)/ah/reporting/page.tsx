...
import { useAhFundingReport, useAhParticipants } from '@/hooks/use-allied-health';
...
export default function CosReportingPage() {
  ...
  const { data: fundingData, isLoading } = useAhFundingReport({
    participantId,
    coordinatorId
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-4 flex flex-wrap gap-4 items-end">
        ...
        <div>
          <label className="block text-sm text-[var(--color-text-muted)] mb-1">Allied Health Worker</label>
          <select
            className="form-input w-48"
            value={coordinatorId}
