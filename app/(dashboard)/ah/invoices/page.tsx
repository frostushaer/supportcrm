...
import { useAhInvoices, useAhParticipants } from '@/hooks/use-allied-health';
...
export default function CosInvoicesPage() {
  ...
  const { data: invoices, isLoading } = useAhInvoices({
    invoiceType,
    participantId
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <PageHeader
          title="Allied Health Invoices"
          description="Manage and track invoices for Allied Health."
        />
        <button className="btn-primary flex items-center gap-2" onClick={() => alert('Add Allied Health Invoice dialog to be implemented')}>
          <Plus className="w-4 h-4" /> Add Allied Health Invoice
        </button>
      </div>
