...
import { useComplaintsReport } from "@/hooks/use-reporting";
...
export default function ComplaintsReportsPage() {
  ...
  const { data: reportData, isLoading } = useComplaintsReport(activeFilters);
  ...
  const handleExport = () => {
    ...
    const rows = reportData.map((row: any) => {
      return [
        ...
        `"${row.name}"`,
        `""`, // category not in complaint schema
        `"${row.status}"`,
        `""` // Assigned to not in schema for complaint yet
      ].join(",");
    });
    ...
    link.setAttribute("download", `Complaints_Report_${format(new Date(), "yyyyMMdd_HHmm")}.csv`);
  ...
  const columns = [
    {
      label: "Date",
      key: "date",
      render: (row: any) => format(new Date(row.date), "dd/MM/yyyy HH:mm")
    },
    {
