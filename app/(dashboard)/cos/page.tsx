...
import { useCosDashboardSummary } from '@/hooks/use-cos';
import Link from 'next/link';
import { DataTable } from '@/components/shared/data-table';


export default function COSDashboard() {
  const { data: summary, isLoading } = useCosDashboardSummary();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Overview of your COS activities and participants."
      />

      <div className="flex flex-wrap gap-4 mb-6">
        <Link href="/cos/participants" className="btn-secondary flex items-center gap-2">
          <Users className="w-4 h-4" /> Participant List
        </Link>
        <Link href="/cos/catalogue-services" className="btn-secondary flex items-center gap-2">
          <ClipboardList className="w-4 h-4" /> Catalogue Service
        </Link>
        <Link href="/cos/service-providers" className="btn-secondary flex items-center gap-2">
          <Building className="w-4 h-4" /> Service Providers
        </Link>
        <Link href="/cos/case-notes-history" className="btn-secondary flex items-center gap-2">
          <History className="w-4 h-4" /> Case Note History
        </Link>
        <Link href="/cos/ndis-claims" className="btn-secondary flex items-center gap-2">
          <CheckCircle className="w-4 h-4" /> NDIS Claims
        </Link>
        <Link href="/cos/invoices" className="btn-secondary flex items-center gap-2">
          <FileText className="w-4 h-4" /> Invoices
        </Link>
        <Link href="/cos/reporting" className="btn-secondary flex items-center gap-2">
