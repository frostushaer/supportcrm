...
export async function PATCH(
  ...
  try {
    ...
    // Verify the note belongs to this participant
    const existingNote = await db.caseNote.findFirst({
      where: { id: noteId, participantId },
    });
    if (!existingNote) {
      return NextResponse.json({ error: 'Case note not found for this participant' }, { status: 404 });
    }
